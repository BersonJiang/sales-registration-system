...
function queryCustomerSpending() {
    ...
    // 只计算现金支付的消费，不包括余额支付和充值记录
    const cashSales = customerSales.filter(sale => sale.paymentType === '现金');
    const totalSpent = cashSales.reduce((sum, sale) => sum + sale.totalAmount, 0);
    ...
    // 获取现金支付购买的商品列表
    const products = cashSales.map(sale => sale.product).join('、');
    
    resultDiv.innerHTML = `
        <h4>客户：${customer}</h4>
        <p><strong>累计消费：</strong>¥${totalSpent.toFixed(2)}</p>
        <p><strong>消费笔数：</strong>${cashSales.length} 笔</p>
        <p><strong>当前余额：</strong>¥${currentBalance.toFixed(2)}</p>
        <p><strong>购买商品：</strong>${products}</p>
    `;
}
